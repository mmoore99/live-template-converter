...
export function parseWebStormTemplate(content: string): WebStormTemplate[] {
  const wrappedContent = content.trim().startsWith('<?xml') || content.trim().startsWith('<templateSet') 
    ? content 
    : `<templateSet>${content}</templateSet>`;
  ...
  return Array.from(templates).map(t => {
    ...
    const options = contextElement 
      ? Array.from(contextElement.getElementsByTagName('option')).map(o => o.getAttribute('name') || '')
      : [];
